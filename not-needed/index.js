...
function main() {
	...
	try {
		...
		const fetchInfo = createFetchServerInfoConfigAndBroadcast({ appCtx, config })
		...
		configManager.on('change', () => {
			...
			if (!forceReload && subsystemReloadSig !== null && nextSig === subsystemReloadSig) {
				logger.info('[Config] Applied file change; subsystem recycle skipped (Caspar / OSC / DMX signature unchanged).')
				return
			}
		...
		const shutdown = Shutdown.createShutdownHandler({ logger, appCtx, moduleRegistry, stopOscSubsystem, wsHandle, httpServer, persistence })
