...
Single entry point for **multi-channel playout**: follow one path below, then use [Audio routing reference](../../reference/audio/audio-routing-reference.md) for MIXER / `route://` / FFmpeg mapping details.

**Related:** WO-06 Audio playout · Phase 5 (T5.1)

| Topic | Deep dive |
|-------|-----------|
| List ALSA devices | [ALSA device enumeration](../../reference/audio/alsa-device-enumeration.md) |
| NVIDIA HDMI/DP, 3.5 mm, USB | [Caspar outputs: NVIDIA, stereo, USB](../../reference/audio/caspar-outputs-nvidia-stereo-usb.md) |
| Consumers in `casparcg.config` | [Caspar audio consumers](../../reference/audio/caspar-audio-consumers.md) |
| AES67, Dante bridges, NDI tools, NetJACK, ROC | [Network audio protocols](../../reference/audio/network-audio-protocols.md) |
| PipeWire sinks / graph | [PipeWire multi-channel routing](../../reference/audio/pipewire-multichannel-routing.md) |
| JACK alternative | [JACK audio routing](../../reference/audio/jack-audio-routing.md) |
...
1. Confirm driver: `lsmod | grep snd_hda_intel` (see [§1.1](../../reference/audio/caspar-outputs-nvidia-stereo-usb.md#11-verify-kernel-audio-stack)).
...
**Also read:** [§1.6 — HDMI/DP into a video switcher](../../reference/audio/caspar-outputs-nvidia-stereo-usb.md#16-hdmi--displayport-into-a-video-switcher-embedded-audio-as-main-output) (embedded audio into switchers that de-embed / bridge to Dante).
...
2. Match **rate** and **channel count** to the device spec (see [§3](../../reference/audio/caspar-outputs-nvidia-stereo-usb.md#3-usb-audio-interface)).
...
**Dante on the network:** Linux has **no** Dante Virtual Soundcard; use a **USB–Dante hardware bridge** or AES67 mode — [Network audio protocols § T3.1](../../reference/audio/network-audio-protocols.md).
...
2. **AES67 mode** on Dante endpoints: configure in **Dante Controller** (Windows/Mac), then use a **Linux AES67/RAVENNA** stack or PipeWire module as described in [§ T3.1](../../reference/audio/network-audio-protocols.md).
...
1. Enable **NDI consumer** on the Caspar channel; set `<name>` uniquely ([§ T2.3](../../reference/audio/caspar-audio-consumers.md#t23--ndi-consumer-network-video--embedded-audio)).
2. Downstream: NDI Studio Monitor, FFmpeg with `libndi_newtek`, or hardware — [§ T3.2](../../reference/audio/network-audio-protocols.md).
...
2. Optional: drop-in **clock/quantum** under `/etc/pipewire/pipewire.conf.d/` ([PipeWire doc](../../reference/audio/pipewire-multichannel-routing.md)).
