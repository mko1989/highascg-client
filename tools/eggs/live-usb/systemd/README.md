...
| **(generated, not `.example`)** | **`scripts/install-exfat-systemd-units.sh`** also writes **`highascg-exfat-media-prep.service`** (creates **`~/exfat/media`**) and **`home-casparcg-highascg-media-exfat.mount`** (bind `~/exfat/media` → `~/highascg/media/exfat`). **`scripts/write-highascg-systemd-unit.sh`** aligns **`highascg.service`** with those units when present. |
